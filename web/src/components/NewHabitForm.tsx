...
export function NewHabitForm() {
  ...
  async function createNewForm(event: FormEvent) {
    event.preventDefault()
    

    if (!title || weekDays.length === 0) {
      alert('Você precisa adicionar um título e selecinar pelo menos um dia da semana!')
      return
    }
  ...
  return (
    <form onSubmit={createNewForm} className="w-full flex flex-col mt-6">
      ...
      <div className="mt-3 flex flex-col gap-2">
        {availableWeekDays.map((weekDay, index) => (
          ...
          >
            <div className="h-8 w-8 flex rounded-lg items-center justify-center bg-zinc-900 border-2 border-zinc-800 group-data-[state=checked]:bg-green-500 group-data-[state=checked]:border-green-500 transition-colors group-focus:ring-2 group-focus:ring-violet-500 group-focus:ring-offset-2 group-focus:ring-offset-zinc-900">
              <Checkbox.Indicator>
